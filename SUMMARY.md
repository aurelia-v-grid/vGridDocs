# Summary

* [Introduction](README.md)
* [01. About](chap01/README.md)
   * [grid columns](chap01/grid_columns.md)
   * [datasource](chap01/datasource.md)
   * [selection](chap01/selection.md)
   * [gridConnector](chap01/gridconnector.md)
* [02. **&ltv-grid&gt** attributes](chap02/README.md)
   * [v-row-height](chap02/v-row-height.md)
   * [v-header-height](chap02/v-header-height.md)
   * [v-footer-height](chap02/v-footer-height.md)
   * [v-panel-height](chap02/v-panel-height.md)
   * [v-grid-connector](chap02/v-grid-connector.md)
   * [v-multi-select](chap02/v-multi-select.md)
   * [v-manual-sel](chap02/v-manual-sel.md)
   * [v-theme](chap02/v-theme.md)
   * [v-row-on-draw](chap02/v-row-on-draw.md)
   * [v-columns](chap02/v-columns.md)
   * [v-i18n](chap02/v-language.md)
   * [v-data-delay](chap02/v-data-delay.md)
* [03. &ltv-grid-col&gt simple](chap03/README.md)
   * [col-width](chap03/col-width.md)
   * [col-type](chap03/col-type.md)
   * [col-field](chap03/col-field.md)
   * [col-sort](chap03/col-sort.md)
   * [col-header-name](chap03/col-header-name.md)
   * [col-filter](chap03/col-filter.md)
   * [col-filter-top](chap03/col-filter-top.md)
   * [col-css](chap03/col-css.md)
   * [col-add-row-attributes](chap03/col-add-row-attributes.md)
   * [col-add-filter-attributes](chap03/col-add-filter-attributes.md)
   * [col-add-label-attributes](chap03/col-add-label-attributes.md)
   * [col-filter-menu](chap03/col-filter-menu.md)
   * [col-label-menu](chap03/col-label-menu.md)
   * [col-row-menu](chap03/col-row-menu.md)
   * [col-hidden](chap03/col-hidden.md)
   * [col-drag-drop (& grouping)](chap03/col-drag-drop.md)
   * [col-resizeable](chap03/col-resizeable.md)
   * [col-pin-left](chap03/col-pin-left.md)
   * [col-pin-right](chap03/col-pin-right.md)
* [04. &ltv-grid-col&gt custom](chap04/README.md)
   * [v-selection](chap04/v-selection.md)
   * [v-sort](chap04/v-sort.md)
   * [v-resize-col](chap04/v-resize-col.md)
   * [v-image-fix](chap04/v-image-fix.md)
   * [v-filter](chap04/v-filter.md)
   * [v-drag-drop-col](chap04/v-drag-drop-col.md)
   * [v-menu](chap04/v-menu.md)
   * [v-filter-trigger](chap04/v-filter-trigger.md)
* [05. &ltv-grid-col&gt custom css help](chap05/README.md)
   * [checkbox](chap05/checkbox.md)
   * [image](chap05/image.md)
   * [input](chap05/input.md)
   * [selection](chap05/selection.md)
* [06. &ltv-grid-row-repeat&gt](chap06/README.md)
* [07. &ltv-grid-context&gt custom menu](chap07/README.md)
   * [&ltv-menu-close&gt](chap07/v-menu-close.md)
   * [&ltv-menu-groupby&gt](chap07/v-menu-groupby.md)
   * [&ltv-menu-sort&gt](chap07/v-menu-sort.md)
   * [&ltv-menu-filter&gt](chap07/v-menu-filter.md)
   * [&ltv-menu-filter-options&gt](chap07/v-menu-filter-options.md)
   * [&ltv-menu-all&gt](chap07/v-menu-all.md)
* [08. &ltv-grid-group-row&gt custom group row](chap08/README.md)
* [09. &ltv-grid-group-element&gt custom group element](chap09/README.md)
* [10. &ltv-grid-footer&gt custom](chap10/README.md)
* [11. &ltv-grid-loading-screen&gt custom](chap11/README.md)
* [15. Install](chap15/README.md)
   * [JSPM](chap15/jspm.md)
   * [Aurelia-CLI (not asp.core)](chap15/aurelia-cli_not_aspcore.md)
   * [Aurelia-CLI (asp-core)](chap15/aurelia-cli_asp-core.md)
   * [Webpack](chap15/webpack.md)

